...
| Hacker News | 技术新闻、创业、开发者讨论 | 官方 Firebase API 或 Algolia HN API | 优先接入 | 数据开放度高，适合技术和创业方向。 |
| GitHub | 热门仓库、近期高星项目、技术趋势 | GitHub REST Search API | 优先接入 | GitHub 没有官方 Trending API，可用搜索条件模拟“近期新增 Star 趋势”。 |
...
一期不要一次性接入所有来源。建议先选择 5 到 7 个来源做稳定闭环，再逐步扩展。
...
- 数据源名称。
- 平台类型：YouTube、X、Reddit、GitHub、Hacker News、Product Hunt、News、Other。
...
- 至少接入 3 个稳定数据源，其中 YouTube、Hacker News、GitHub 建议作为首批。
- 支持每日定时采集和手动采集。
- 采集任务有成功、失败、入库、去重记录。
- 热点列表可按日期、平台、主题筛选。
